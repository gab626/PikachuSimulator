...
def main():
    ...
    def phase(f, r, l, c, rgen, rl): # MANCA FONDO COSTANTE
        w = 2 * np.pi * f
        y = 1 - w**2 * l * c
        rtot = r + rgen
        num = w * (rl**2 * c - y * l)
    ...
    P1 = [r1, l, c, rgen, rl] # parametri liberi per i fit
    P2 = [r2, l, c, rgen, rl]
    P3 = [r3, l, c, rgen, rl]
    ...
    plt.title("Phase fit R1", fontsize=30.0, fontname='sans-serif')
    ...
    plt.xlabel("frequenza (Hz)", fontsize=20.0)
    plt.ylabel("fase (rad)", fontsize=20.0)
    plt.title("Phase fit R2", fontsize=30.0, fontname='sans-serif')
    plt.legend(loc='upper left', fontsize=14.0, markerscale=2.0)
    plt.grid(True)
    ...
    plt.xlabel("frequenza (Hz)", fontsize=20.0)
    plt.ylabel("fase (rad)", fontsize=20.0)
    plt.title("Phase fit R3", fontsize=30.0, fontname='sans-serif')
    plt.legend(loc='upper left', fontsize=14.0, markerscale=2.0)
    plt.grid(True)
